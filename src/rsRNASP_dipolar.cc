...
std::pair<double, double> rsRNASP_dipolar::calc_rsRNASP_dipo(rna &astr){
                    ...
                    for(auto & aatom : ares.atoms){
                        ...
                        for (auto & batom: bres.atoms){
                            ...
                            if (aatom.is_polar()){
                                ...
                                printf("pn %d %s %d %s %8.4f\n", ares.get_residsd(), aatom.get_name().c_str(), bres.get_residsd(), batom.get_name().c_str(), rsRNASP_dipo_pn[atype_ia][atype_ib][abin][bin]);

                                #endif

                                etotal_pn = etotal_pn + rsRNASP_dipo_pn[atype_ia][atype_ib][abin][bin];
                            ...
                            if (batom.is_polar()){
                                ...
                                printf("np %d %s %d %s %8.4f\n", ares.get_residsd(), aatom.get_name().c_str(), bres.get_residsd(), batom.get_name().c_str(), rsRNASP_dipo_pn[atype_ib][atype_ia][abin][bin]);

                                #endif

                                // count_dipo_pn[atype_ib][atype_ia][abin][bin]++;
                                etotal_pn = etotal_pn + rsRNASP_dipo_pn[atype_ib][atype_ia][abin][bin];
                            }
                            ...
                            if (aatom.is_polar() && batom.is_polar()){
                                ...
                                printf("pp %d %s %d %s %8.4f\n", ares.get_residsd(), aatom.get_name().c_str(), bres.get_residsd(), batom.get_name().c_str(), rsRNASP_dipo_pp[atype_ia][atype_ib][abbin][bin]);

                                #endif

                                etotal_pp = etotal_pp + rsRNASP_dipo_pp[atype_ia][atype_ib][abbin][bin];
...
void rsRNASP_dipolar::init_rsRNASP_etable_dipo(string energy_dir){
    std::string rsRNASP_file1 = energy_dir + "/energy.dipo.pn.dat";
    std::string rsRNASP_file2 = energy_dir + "/energy.dipo.pp.dat";

    if (! misc::file_exists(rsRNASP_file1)){
        cerr << "# File not found: " << rsRNASP_file1.c_str() << endl;
        exit(1);
    }
    if (! misc::file_exists(rsRNASP_file2)){
        cerr << "# File not found: " << rsRNASP_file2.c_str() << endl;
        exit(1);
    }

    std::ifstream fs(rsRNASP_file1);
    std::string line;
    string ss[3];
...
// pn
    ...
    while (std::getline(fs, line)){
        ...
        for (int id_mbin = 0; id_mbin < mbin; id_mbin ++){
            iss >> rsRNASP_dipo_pn[id1][id2][id_dipo][id_mbin];
        }
    }
...
// pp
    ...
    std::ifstream fs2(rsRNASP_file2);
    while (std::getline(fs2, line)){
        ...
        for (int id_mbin = 0; id_mbin < mbin; id_mbin ++){
            iss >> rsRNASP_dipo_pp[id1][id2][id_dipo][id_mbin];
        }
    }
...
void rsRNASP_dipolar::init_dipo_arrays(){
        ...
        for (index_d j = 0; j < polartypen; j ++)
            for (index_d k = 0; k < dipo_bin; k ++)
                for (index_d l = 0; l < mbin;l ++){
                    ...
                    rsRNASP_dipo_pp[i][j][k][l] = 0;
                }
}

void rsRNASP_dipolar::train_rsRNASP_dipo(std::string pdbl_fpath, int length_limit){
    ...
    count_to_rsRNASPdipo();
}




void rsRNASP_dipolar::count_dipo(rna &astr){
...
void rsRNASP_dipolar::count_to_rsRNASPdipo(){
...
void rsRNASP_dipolar::print_count_dipo(FILE * out_pn, FILE * out_pp){
...
void rsRNASP_dipolar::print_count_dipo(std::string & sout_pn, std::string & sout_pp){
    FILE * out_pn;
...
void rsRNASP_dipolar::print_ersRNASP_dipo(std::string & sout_pn, std::string & sout_pp){
    FILE * out_pn;
    out_pn = fopen(sout_pn.c_str(),"w");
    ...
    print_ersRNASP_dipo(out_pn, out_pp);

    fclose(out_pn);
    fclose(out_pp);
}

void rsRNASP_dipolar::print_ersRNASP_dipo(FILE * out_pn, FILE * out_pp){

    for (index_d i = 0; i < rsRNASP_dipo_pn.shape()[0]; i++){
        for(index_d j = 0; j < rsRNASP_dipo_pn.shape()[1]; j++){
            for (index_d k = 0; k < rsRNASP_dipo_pn.shape()[2]; k++){
                fprintf(out_pn,"%s\t%s\t%d",pdb_utils::typepolar[i].c_str(),pdb_utils::typeatom[j].c_str(),int(k));
                for (index_d l = 0; l < rsRNASP_dipo_pn.shape()[3]; l++){
                    fprintf(out_pn,"\t%8.6f",rsRNASP_dipo_pn[i][j][k][l]);
                }
                fprintf(out_pn,"\n");
            }
    ...
    for (index_d i = 0; i < rsRNASP_dipo_pp.shape()[0]; i++){
        for(index_d j = 0; j < rsRNASP_dipo_pp.shape()[1]; j++){
            for (index_d k = 0; k < rsRNASP_dipo_pp.shape()[2]; k++){
                fprintf(out_pp,"%s\t%s\t%d",pdb_utils::typepolar[i].c_str(),pdb_utils::typepolar[j].c_str(),int(k));
                for (index_d l = 0; l < rsRNASP_dipo_pp.shape()[3]; l++){
                    fprintf(out_pp,"\t%8.6f",rsRNASP_dipo_pp[i][j][k][l]);
                }
                fprintf(out_pp,"\n");
            }
...
int rsRNASP_dipolar::ang2bin(double angle){
    int angle_bin=-1;
...
int rsRNASP_dipolar::cosang2bin(double cos_angle){
    int angle_bin=-1;
    angle_bin = (int) ((cos_angle + 1.0)/ (1.0/3));
    if (angle_bin == 6){
        angle_bin = 5;
